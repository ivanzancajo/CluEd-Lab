...
function setupTerminal(position: { positionX: number; positionY: number }, hasMoved: boolean) {
  const { session, team } = buildSession(position, hasMoved);
...
describe("lógica de pasadizo según turno", () => {
  describe("turno de entrada en sala este turno (hasMoved: true)", () => {
    beforeEach(() => {
      setupTerminal(CORNER_ROOM_POSITION, true);
    });

    it("oculta el botón de pasadizo aunque el equipo esté en una sala esquina con pasadizo", () => {
  ...
  describe("turno con sala desde el anterior (hasMoved: false)", () => {
    ...
    it("muestra el botón de pasadizo cuando el equipo lleva en sala desde el turno anterior", () => {
      cy.get('[data-cy="terminal-themed-board"]').should("be.visible");
      cy.get('[data-cy="terminal-secret-passage-emit"]').should("exist");
    });

    it("el botón de pasadizo muestra la sala destino correcta", () => {
      cy.get('[data-cy="terminal-secret-passage-emit"]').should("contain.text", "Sala inferior derecha");
    });

    it("el botón de pasadizo está deshabilitado sin conexión realtime activa", () => {
      cy.get('[data-cy="terminal-secret-passage-emit"]').should("be.disabled");
    });
  });

  describe("equipo fuera de sala con pasadizo (sin botón posible)", () => {
    beforeEach(() => {
      setupTerminal(SPAWN_POSITION, false);
    });

    it("no muestra el botón de pasadizo cuando el equipo está en un spawn", () => {
      cy.get('[data-cy="terminal-themed-board"]').should("be.visible");
      cy.get('[data-cy="terminal-secret-passage-emit"]').should("not.exist");
    });

    it("no muestra el botón de pasadizo aunque hasMoved sea false fuera de sala esquina", () => {
      cy.get('[data-cy="terminal-secret-passage-emit"]').should("not.exist");
    });
  });
});
